...
class MainWin:
    def __init__(self):        
        ...
        self.__connectMenu.add_command(label='연결종료', command=self.disconnect, state='disable')
        ...
        self.__sendMessage = tkinter.Entry(frameAction, width=50, state='disabled')
        self.__sendMessage.pack(expand=True, side='left', fill='both')
        self.__btnSend = tkinter.Button(frameAction, text='전송', width=10, state='disabled', command=self.send_message)
    ...
    def open_logwin(self):
        ...
        labelHost = tkinter.Label(frameHost, text='서버주소', width=8, justify='center')    
        ...
        labelPort = tkinter.Label(framePort, text='포트번호', width=8, justify='center')
        ...
        labelNick = tkinter.Label(frameNick, text='닉네임', width=8, justify='center')
        ...
        btnCancel = tkinter.Button(frameAction, text='취소하기', command=self.disconnect)    
